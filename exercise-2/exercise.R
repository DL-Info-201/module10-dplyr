...
# Save this new data frame in a variable.
  ...
  two.wheel.20 <- filter(vehicles, drive == "2-Wheel Drive", hwy > 20)

# Of those vehicles, what is the vehicle ID of the vehicle with the worst hwy mpg?
# Hint: filter for the worst vecle, then select its ID.
  
  worst.hwy <- filter(two.wheel.20, hwy == min(hwy))
  worst.hwy.id <- select(worst.hwy, id)
...
# You'll need to filter more!
  ...
  MostHwyMPG <- function(my_make, my_year) {
    year.and.make.cars <- filter(vehicles, year == my_year, make == my_make)
    return(filter(year.and.make.cars, hwy == max(hwy)))
  }


# What was the most efficient honda model of 1995?
  print(MostHwyMPG("Honda", 1995))
