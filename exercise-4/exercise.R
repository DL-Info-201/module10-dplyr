...
# Note: use the `na.rm = TRUE` argument to remove NA values from your aggregation
  ...
  average.time <- mean(flights$time_gained, na.rm = TRUE)
  print(average.time)

# Create a data.frame of flights headed to seatac ('SEA'), only including the
# origin, destination, and the "gain" column you just created
  
  seatac <- filter(flights, dest == "SEA") %>% select(origin, dest, time_gained)

# On average, did flights to seatac gain or loose time?
  
  avg.seatac <- mean(seatac$time_gained, na.rm = TRUE)
  
# Consider flights from JFK to SEA. What was the average, min, and max air time
# of those flights? Bonus: use pipes to answer this question in one statement!
# (and don't show any other data)
  
  jfk.seatac <- filter(seatac, origin == "JFK") %>% summarise(mean.air = mean(time_gained, na.rm = TRUE),
                                                              min.air = min(time_gained, na.rm = TRUE),
                                                              max.air = max(time_gained, na.rm = TRUE))
  View(jfk.seatac)